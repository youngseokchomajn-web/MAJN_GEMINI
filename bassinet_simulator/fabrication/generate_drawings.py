...
코어 450x250x18mm / 자작합판 4mm / 측판 10mm 샌드위치 (보고서 §7, 커밋 d7668df)
...
T = 4.0                       # 합판 두께
WALL_H = 10.0                 # 측판 높이(=내부 유효고)
TOTAL_H = T + WALL_H + T      # 18.0
...
EXC_FOOT = (37.0, 20.0)       # TEAX14C02-8 풋프린트
HOLE_D = 3.2                  # M3 관통홀
CSK_D = 6.5                   # 상판 카운터싱크(90°) 참고 지름
# 체결 블록(각재 20x20x10) 중심: 코너 4 + 변 중앙 4 — 벽 안쪽 밀착(벽 4 + 블록 반폭 10 = 14)
...
wy = 2 * (L + GAP)
rect(0, wy, *WALL_FB)
label(0, wy - 12, "WALL-FRONT 450x10x4")
...
label(0, y0 - 12, "WALL-REAR 450x10x4 (케이블 노치 10x6 @중앙)")
wy2 = y0 + WALL_H + GAP
rect(0, wy2, *WALL_LR)
label(0, wy2 - 12, "WALL-LEFT 242x10x4")
rect(WALL_LR[0] + GAP, wy2, *WALL_LR)
label(WALL_LR[0] + GAP, wy2 - 12, "WALL-RIGHT 242x10x4")
label(0, wy2 + WALL_H + 18,
      "MAJN NUCU PAD v3 CORE 450x250x18 | BIRCH PLY 4mm | CUT=red DRILL=blue REF=green(NO CUT)", h=7)
label(0, wy2 + WALL_H + 32,
      "별도자재: 체결블록 각재 20x20x10 x8 (D3.2 관통) / M3x22 접시볼트+너트 x8 / 목공본드 / EVA보더 별도", h=5)
...
parts = []
parts.append('<g>')
parts.append(svg_text(OX + W/2, 22, "MAJN 누쿠 패드 v3 — 진동 코어 450×250×18mm 제작 도면 (Rev.A / 2026-08-04)", "title"))
...
parts.append(svg_text(x0 + W/2, y0 + L/2 + 4, "평면도 — 상판 안쪽면 (익사이터 VHB 존 37×20, 홀 8×⌀3.2)", "cap"))
...
svg_parts.append(vdim(sy + T, sy + T + WALL_H, sx - 8, "10"))
svg_parts.append(vdim(sy + T + WALL_H, sy + TOTAL_H, sx - 8, "4"))
svg_parts.append(vdim(sy, sy + TOTAL_H, sx - 22, "18"))
# 볼트 상징선
svg_parts.append(f'<line x1="{P(sx+T+10):.1f}" y1="{P(sy-3):.1f}" x2="{P(sx+T+10):.1f}" y2="{P(sy+TOTAL_H+3):.1f}" class="bolt"/>')
svg_parts.append(svg_text(sx + T + 10, sy + TOTAL_H + 9, "M3×22 CSK", "small"))
...
rows = [
    ...
    ("③", "전면 측판", "450 × 10 × 4", "1", ""),
    ("④", "후면 측판", "450 × 10 × 4", "1", "케이블 노치 10×6 중앙"),
    ("⑤", "좌/우 측판", "242 × 10 × 4", "2", ""),
    ("⑥", "체결 블록", "20 × 20 × 10 각재", "8", "⌀3.2 관통, 별도 재단"),
    ("⑦", "익사이터", "TEAX14C02-8", "4", "VHB 9473PC, 상판 안쪽면"),
    ("⑧", "볼트/너트", "M3×22 접시 + 너트", "8", "0.8 N·m"),
    ("⑨", "EVA 보더", "두께 18, 경도 60~70C", "-", "배시넷 바닥 실측 후 재단"),
]
...
notes = [
    "1. 소재: 자작합판 4.0mm (E≈9.15GPa · 항복 48MPa 급). 전 부품 동일 시트에서 재단.",
    "2. 조립: 목공본드 전둘레 + 체결블록⑥ 8개소 M3×22 관통볼트(상면 접시, 토크 0.8N·m).",
    "3. 익사이터 4개 VHB로 [상판 안쪽면] 지정 위치 부착 → 배선 직렬 16Ω/채널, 후면 노치로 인출.",
    "4. 마감: 전면 방수 실링 + 어린이제품 유해물질 기준 도료. 외곽 모서리 R2 이상 라운드.",
    "5. 검수: 총높이 18.0±0.3 · 15kg 중앙 정적 휨 ≤2.5mm (실측 프로토콜 v1.1 G1).",
