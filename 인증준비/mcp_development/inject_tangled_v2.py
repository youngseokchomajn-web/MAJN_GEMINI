...
try {
    ...
    // 2. Place components using lib_Device
    let compsToPlace = [
        ...
        try {
            ...
            if (devices && devices.length > 0 && devices[0]) {
                ...
                if (primId) {
                    ...
                    let comp = allComps.find(x => x.getState_PrimitiveId() === primId);
                    if (comp) {
                        await comp.setState_Designator(c.des);
                        await comp.done();
                    }
                ...
                result.push("Device not found for " + c.lcsc);
            }
    ...
    // 3. Assign Nets
    let allComps = await eda.pcb_PrimitiveComponent.getAll();
    const getCompByDes = (des) => allComps.find(c => c.getState_Designator() === des);
