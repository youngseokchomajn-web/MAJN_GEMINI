...
try {
    ...
    // Get CURRENT PCB active in user's editor
    ...
    // Workaround bug: Create a tiny dummy region to initialize internal attrsMap
    try {
        const dummySource = ['R', 0, 0, 10, 10, 0, 0];
        const dummyPoly = eda.pcb_MathPolygon.createPolygon(dummySource);
        if (dummyPoly) {
            await eda.pcb_PrimitiveRegion.create(11, dummyPoly, [], "Dummy Init");
            result.push("Created Dummy Region to init attrsMap");
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
    } catch(e) { result.push("Region err: " + e.message); }
    ...
    try {
        ...
        if (config) {
            ...
            result.push("DRC set to 0.1mm");
        }
    } catch(e) {}
    
    // Force hydrate LCSC components
