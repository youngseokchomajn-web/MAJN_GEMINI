...
def padbox(p):
    nums = [v for v in p[6][1:] if isinstance(v, (int, float))] if isinstance(p[6], list) else []
...
def components(NET, plane=False):
    ...
    for v in V:
        if v[0] == NET: objs.append(('via', '', 12, ('via', v[1], v[2], v[4])))
    PL = None
    if plane:
    ...
    if plane:
        disc = [objs[i][1] for i in range(N) if objs[i][0] == 'pad' and find(i) != find(PL)]
        return disc
...
print('=== 연결 수정 재검증 (저장본) ===')
for net in ['AMP_OUT_B+', 'AMP_ADR', 'VCC_3V3']:
    pc = components(net)
    print('  %-9s → %s' % (net, '✅ 1성분 연결' if len(pc) <= 1 else '❌ %d성분 %s' % (len(pc), pc)))
disc = components('GND', plane=True)
print('  %-9s → %s' % ('GND', '✅ 전 패드 평면연결' if not disc else '❌ 미연결 ' + str(disc)))
