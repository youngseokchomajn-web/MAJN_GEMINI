...
try {
    ...
    // Modify safe spacing values to 0.1mm
    ...
    // Also modify default track width to 0.15mm to be safe
    try {
        config.config.Track.Track["Track Width"].tables["1"].content[0] = 0.15; // Min
        config.config.Track.Track["Track Width"].tables["1"].content[1] = 0.15; // Preferred
    } catch(e) {}
    
    await eda.pcb_Drc.overwriteCurrentRuleConfiguration(config);
    return "Rules updated to 0.1mm clearance successfully!";
} catch(e) {
    return e.message;
}
"""
